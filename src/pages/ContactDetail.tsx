...
import { 
  ...
  MapPin, User, Tag, CheckCircle2, MoreVertical, Plus
} from 'lucide-react';
    ...
    try {
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          url: publicUrl,
          size: file.size,
          uploaded_by: 'Jeffrey Newell' // Mock user for now
        } as any);
    ...
    <div className="h-full flex flex-col bg-slate-50">
      ...
      <div className="bg-primary text-white p-6 pb-20 relative">
        ...
        <div className="absolute -bottom-8 left-6 right-6 flex justify-between gap-3">
          {[
            { icon: Phone, label: 'Call', color: 'bg-emerald-500' },
            { icon: MessageSquare, label: 'SMS', color: 'bg-accent' },
            { icon: Mail, label: 'Email', color: 'bg-amber-500' },
          ].map((action) => (
            <button key={action.label} className="flex-1 bg-white p-4 rounded-2xl shadow-lg flex flex-col items-center gap-1 active:scale-95 transition-transform">
              <div className={`${action.color} h-10 w-10 rounded-xl flex items-center justify-center text-white mb-1`}>
                <action.icon size={20} />
              </div>
            ...
            return (
              <button
                ...
                className={`py-4 flex items-center gap-2 border-b-2 transition-all ${
                  isActive ? 'border-accent text-accent' : 'border-transparent text-slate-400'
                }`}
              >
                <Icon size={18} />
      ...
      <div className="flex-1 overflow-y-auto p-6 no-scrollbar">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'inspection' && <InspectionTab contact={contact} />}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-1">
          {STAGES.map((stage, i) => {
            ...
            return (
              <div 
                key={stage} 
                className={`h-1.5 flex-1 rounded-full ${isCompleted ? 'bg-accent' : 'bg-slate-100'}`} 
              />
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                {contact.phone2 && (
                  <p className="text-xs text-slate-500 mt-1">Secondary: {formatPhone(contact.phone2)}</p>
                )}
              </div>
            </div>
            <div className="flex items-start gap-4">
        ...
        <div className="card p-5 space-y-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Assignment & Tags</h3>
          <div className="flex items-center gap-4">
            <div className="h-10 w-10 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden">
              <User size={20} className="text-slate-400" />
            </div>
            <div className="flex-1">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Assigned Rep</p>
              <p className="text-sm font-bold text-primary">Jeffrey Newell</p>
            </div>
            <button className="text-accent text-xs font-bold">Change</button>
          </div>
          <div className="pt-4 border-t border-slate-50">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button className="bg-slate-50 text-slate-400 p-1 rounded-md">
                <Plus size={14} />
              </button>
            </div>
          </div>
...
function InspectionTab({ contact }: { contact: any }) {
...
Steepness: ${checklist.steepness}`;
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          type: 'note',
          content: content,
          user_id: 'Jeffrey Newell', // Mock user
          direction: 'outbound'
        } as any);
  ...
  const calculatePitch = () => {
    const angle = (Math.atan(pitch.rise / pitch.run) * 180) / Math.PI;
      ...
      <div className="card p-5 bg-slate-900 text-white space-y-4">
        ...
        <div className="flex items-center justify-around py-4">
          <div className="text-center">
            ...
            <input 
              type="number" 
              className="w-16 bg-white/10 border-none rounded-lg text-center font-bold text-xl p-2"
              value={pitch.rise}
              onChange={(e) => setPitch({...pitch, rise: parseInt(e.target.value) || 0})}
            />
          </div>
        ...
        <div className="flex gap-2">
          {[4, 6, 8, 10, 12].map(r => (
            <button 
              key={r}
              onClick={() => setPitch({...pitch, rise: r})}
              className={`flex-1 py-2 rounded-lg text-xs font-bold transition-colors ${pitch.rise === r ? 'bg-accent text-white' : 'bg-white/5 text-slate-400'}`}
            >
              {r}/12
            </button>
          ))}
      ...
      <div className="card p-5 space-y-4">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select 
              className="w-full bg-slate-50 border-none rounded-xl p-3 text-sm mt-1"
              value={checklist.roofAge}
              onChange={(e) => setChecklist({...checklist, roofAge: e.target.value})}
            >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              {['Shingle', 'Metal', 'Tile', 'Flat'].map(m => (
                <button 
                  key={m}
                  onClick={() => setChecklist({...checklist, material: m})}
                  className={`p-3 rounded-xl text-xs font-bold border transition-all ${
                    checklist.material === m ? 'bg-accent border-accent text-white' : 'bg-white border-slate-100 text-slate-600'
                  }`}
                >
                  {m}
                </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              {['Hail', 'Wind', 'Wear', 'None'].map(d => (
                <button 
                  key={d}
                  onClick={() => setChecklist({...checklist, damageType: d})}
                  className={`p-3 rounded-xl text-xs font-bold border transition-all ${
                    checklist.damageType === d ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-100 text-slate-600'
                  }`}
                >
                  {d}
                </button>
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
            ...
            <button 
              onClick={() => setChecklist({...checklist, leaks: !checklist.leaks})}
              className={`w-12 h-6 rounded-full transition-colors relative ${checklist.leaks ? 'bg-emerald-500' : 'bg-slate-300'}`}
            >
        ...
        <button 
          ...
          disabled={!checklist.material || !checklist.damageType}
          ...
          return (
            <div key={stage} className="flex gap-6 relative">
              <div className={`h-8 w-8 rounded-full flex items-center justify-center z-10 ${
                isDone ? 'bg-accent text-white' : 'bg-white border-2 border-slate-100 text-slate-300'
              }`}>
                {isDone ? <CheckCircle2 size={16} /> : <div className="h-2 w-2 rounded-full bg-current" />}
              </div>
              <div className="flex-1 pt-1">
                <p className={`text-sm font-bold ${isDone ? 'text-primary' : 'text-slate-400'}`}>
                  {stage.replace('_', ' ').toUpperCase()}
                </p>
                {isDone && <p className="text-[10px] text-slate-400">Completed Mar 16, 2026</p>}
              </div>
            </div>
          );
        })}
...
function TimelineTab({ timeline }: { timeline: any[] }) {
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-bold text-primary">Communications</h3>
        <button className="bg-accent text-white p-2 rounded-xl">
          <Plus size={18} />
        </button>
      </div>
      <div className="space-y-4">
  ...
  return (
    <div className="space-y-6">
      <div className="bg-primary/5 border border-primary/10 rounded-2xl p-5 space-y-3">
        ...
        <button className="w-full bg-primary text-white py-3 rounded-xl text-xs font-bold active:scale-95 transition-transform">
          Order Aerial Measurement
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="card p-5 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="bg-emerald-100 text-emerald-600 text-[10px] font-bold px-2 py-1 rounded-md uppercase">Paid</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xl font-bold text-primary">$2,500.00</span>
            <span className="text-xs text-slate-400">Mar 10, 2026</span>
          </div>
        </div>
        ...
        <div className="card p-5 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="bg-slate-100 text-slate-400 text-[10px] font-bold px-2 py-1 rounded-md uppercase">Pending</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xl font-bold text-primary">$10,000.00</span>
            <button className="bg-accent text-white px-3 py-1.5 rounded-lg text-[10px] font-bold">Mark Paid</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="card p-5 space-y-4">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm font-bold text-primary">{contact.insurance_company || 'State Farm'}</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase">Claim #</p>
              <p className="text-sm font-bold text-primary">{contact.claim_number || 'SF-99283-X'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase">Deductible</p>
              <p className="text-sm font-bold text-primary">${contact.deductible || '1,000'}</p>
            </div>
          </div>
        </div>
      </div>
